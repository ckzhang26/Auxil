...
class _VeterinaryPageState extends State<VeterinaryPage> {
  ...
  Future<void> _getNearbyVetHospitals() async {
    ...
    print(city);
    final url = Uri.parse(
        "https://maps.googleapis.com/maps/api/place/nearbysearch/json?keyword=veterinary_care&location=" +
            latLng +
            '&radius=' +
            radius +
            '&key=' +
    ...
    if (vetResponse.statusCode == 200) {
      nearbyVetsResponse =
          NearbyVetHospitals.fromJson(jsonDecode(vetResponse.body));
      ...
      for (int i = 0; i < nearbyVetsResponse.results!.length; i++) {
        if (nearbyVetsResponse.results![i].geometry!.location!.lat != null &&
            nearbyVetsResponse.results![i].geometry!.location!.lng != null) {
                  ...
                  child: ListView(
                    children: [
                      if (nearbyVetsResponse != null)
                        for (int i = 0;
                            i < nearbyVetsResponse.results!.length;
                            i++)
